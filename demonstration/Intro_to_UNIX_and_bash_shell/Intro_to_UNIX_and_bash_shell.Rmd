...
Most of the materials originate from the [Introduction to the Unix Command Line](https://cambiotraining.github.io/unix-shell/) with some of the text copied with a few edits.
...
We can specify the arguments `-l` (l for list) and `-a` (a for all) to list all files in a long listing format with additional information.

```{bash}
ls -la
```

We can notice that we have two special files, `.` and `..`, where `.` points to the current directory and `..` points to the parent directory of the current directory. So, `ls -la` is equivalent to `ls -la .`.
...
We can notice that we are still in the current directory instead of the data directory. This is because the command only works within a code chunk. So, remember to execute the commands in the command line instead of this .Rmd file.
...
The `test_data/` directory contains a file called `test_file.txt`, which we want to move to `my_folder/`. We can use the `mv` ("move") command:
...
The `cp` command works similarly to `mv`, except it copies a file instead of moving it. For example, let’s make a copy of our `my_test_file_1.txt` file:
...
For example typing `ls test_data/*.pdf` in the `test_data/` directory (which does not contain any PDF files) results in a message that there are no files called `*.pdf`.
...
Retrieve the sorted, de-duplicated fifth column from the `example.csv` file, then filter to keep only the entries that contain the word '27'.
...
Write a new script called `count_input.sh` that accepts two parameters: the first parameter as a pattern (e.g., "200") to be counted, and the second parameter as the path of the file to operate on.
...
and `example_sub3.csv`. But what if we had hundreds (or thousands!) of these files? We’ll use a loop to solve this problem, but first let’s look at the general form of a loop:
...
Write a script called `count_input_loop.sh` that takes one parameter as a pattern (e.g., "200") and calls `count_input.sh` on `example_sub1.csv`, `example_sub2.csv`, and `example_sub3.csv` files using the input pattern.
...
```         
#!/bin/bash
# assign the first parameter to the variable 'pattern'
pattern=$1
# assign the second parameter to the variable 'file'
file=$2
